...
Future<void> navEmployee(tester) async {
  final employeeButton = find.byKey(const Key('employees-nav'));
  await tester.tap(employeeButton);
  await tester.pumpAndSettle();
}

Future<void> navProjects(tester) async {
  final projectsButton = find.byKey(const Key('projects-nav'));
  await tester.tap(projectsButton);
  await tester.pumpAndSettle();
}

Future<void> navReport(tester) async {
  final reportButton = find.byKey(const Key('report-nav'));
